...
def average_perceptron(feature_matrix, labels, T):
    ...
    theta_store = np.zeros(feature_matrix.shape[1])
    theta_0_store = 0
    ...
    for _ in range(T):
        for i in get_order(feature_matrix.shape[0]):
            if labels[i] * (np.dot(theta, feature_matrix[i, :]) + theta_0) <= 0:
                theta, theta_0 = perceptron_single_step_update(feature_matrix[i, :], labels[i], theta, theta_0)
            theta_store += theta
            theta_0_store += theta_0
    theta_final = theta_store / (num_points * T)
    theta_0_final = theta_0_store / (num_points * T)
...
def pegasos(feature_matrix, labels, T, L):
    ...
    theta = np.zeros(feature_matrix.shape[1])
    theta_0 = 0
    t_all = [i for i in range(1, feature_matrix.shape[0] * T + 1)]
